...
function processDispatchQueueItemsInOrder(
  ...
  if (inCapturePhase) {
    for (let i = dispatchListeners.length - 1; i >= 0; i--) {
      ...
      //执行事件监听函数_currentTarget为触发的dom节点
...
export function listenToAllSupportedEvents(rootContainerElement: EventTarget) {
  if (!(rootContainerElement: any)[listeningMarker]) { //已经绑定过了事件则跳过
    (rootContainerElement: any)[listeningMarker] = true;//添加已绑定事件标识符
    //allNativeEvents 是一个 set 集合，保存了 81 个浏览器常用事件。
    allNativeEvents.forEach(domEventName => {
      // We handle selectionchange separately because it
      // doesn't bubble and needs to be on the document.
      if (domEventName !== 'selectionchange') {
        //nonDelegatedEvents这个也是一个集合，保存了浏览器中不会冒泡的事件，一般指的是媒体事件，比如 pause，play，playing 等，还有一些特殊事件，比如 cancel ，close，invalid，load，scroll 。
        if (!nonDelegatedEvents.has(domEventName)) {
