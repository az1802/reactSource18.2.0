...
function FiberNode(
  ...
  // Instance
  this.tag = tag; //fiber节点类型 classComponent  FunctionComponent 标记
  this.key = key;// fiber节点上的key标记,diff算法时使用
  this.elementType = null;//reactElement 类型
  this.type = null; //React.createElement()函数
  ...
  this.ref = null; //ref值,layout阶段完成之后进行ref赋值
  this.refCleanup = null;

  this.pendingProps = pendingProps; //准备更新的props
  this.memoizedProps = null;//旧的props
  this.updateQueue = null; //
  this.memoizedState = null; //旧的state hooks则为 effect List
  this.dependencies = null;

  this.mode = mode; //fiber节点更新的模式

  // Effects
  this.flags = NoFlags; //effect类型
  this.subtreeFlags = NoFlags; //子节点的effect 类型
  this.deletions = null; //删除

  this.lanes = NoLanes; //任务等级
  this.childLanes = NoLanes; //子节点的任务等级

  this.alternate = null; //对应的另一个缓存树的fiber节点

  if (enableProfilerTimer) { //渲染的性能时间
