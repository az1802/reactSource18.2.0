...
export function requestUpdateLane(fiber: Fiber): Lane {
  ...
  if ((mode & ConcurrentMode) === NoMode) { //同步更新模式
  ...
  const isTransition = requestCurrentTransition() !== NoTransition; //startTrasition阶段
  if (isTransition) {
...
function ensureRootIsScheduled(root: FiberRoot, currentTime: number) {
  ...
  //给当前root的更新优先级,绑定到最新的优先级,同时设置回调任务
...
function performUnitOfWork(unitOfWork: Fiber): void {
  ...
  if (next === null) { //当前fiber节点和子节点已经处理完毕
    // If this doesn't spawn new work, complete the current work.
    completeUnitOfWork(unitOfWork);
  } else {
    workInProgress = next; //处理下一个fiber节点
  }
...
// 避免嵌套更新  setState inside useEffect 可能形成循环触发render函数
