...
export const ReplaceState = 1; //TODO  替换更新
...
//更新任务压入到fiber节点的更新任务队列中
export function enqueueUpdate<State>(
  fiber: Fiber, //fiber节点
  update: Update<State>,//更新任务
  lane: Lane,
): FiberRoot | null {
  const updateQueue = fiber.updateQueue; //fiber节点上的更新队列
  if (updateQueue === null) {
    // Only occurs if the fiber has been unmounted. 只有被unmount updateQueue会被设置为null
    return null;
  }
  ...
  if (isUnsafeClassRenderPhaseUpdate(fiber)) {
    ...
    const pending = sharedQueue.pending; //update
    if (pending === null) {
