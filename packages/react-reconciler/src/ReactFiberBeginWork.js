...
function updateContextProvider(
  ...
  /*  获取 Provder 上的 value ,主要是已处理provider 嵌套的问题,通过一个栈来保存当前的provider value 子节点取的时候会取最近的value */
    ...
    if (oldProps !== null) {
      ...
      if (is(oldValue, newValue)) {
        ...
        if (
          ...
          //context value未发生变化,停止调和子节点,直接return
          return bailoutOnAlreadyFinishedWork(
            current,
            workInProgress,
            renderLanes,
...
function checkScheduledUpdateOrContext(
  ...
  //当前 fiber 的 lane 是否等于当前的更新优先级 则更新来源当前 fiber
...
//处理部分 Context 逻辑，但是最重要的是调用了 bailoutOnAlreadyFinishedWork
...
//开始进行调和
function beginWork(
  current: Fiber | null,  //current 树fiber
  workInProgress: Fiber, //workInProgress 树 fiber
  renderLanes: Lanes, //当前的render优先级
): Fiber | null {
  if (__DEV__) {
  ...
  if (current !== null) {
    const oldProps = current.memoizedProps; //上一次渲染后的props
    const newProps = workInProgress.pendingProps; //这一次更新的props
  ...
  workInProgress.lanes = NoLanes;
  // 走到这里流程会被调和|更新,比如函数执行会执行,类组件会执行render
  switch (workInProgress.tag) {
