...
export function createHydrationContainer(
  ...
  const current = root.current; //当前的rootFiber节点
  const eventTime = requestEventTime(); //TODO 根据时间进行任务调度
  const lane = requestUpdateLane(current);//更新任务的等级
  const update = createUpdate(eventTime, lane);//根据等级创建任务
  //更新完成之后的回调
  update.callback =
...
export function updateContainer(
  ...
  container: OpaqueRoot, //fiberRoot节点
  ...
  const current = container.current; //提取当前rootFiber
  ...
  const context = getContextForSubtree(parentComponent); //获取子节点的context

  if (container.context === null) {
    container.context = context;
  } else {
    container.pendingContext = context; //已存在context pendingContext表示待更新context
  }
  ...
  update.payload = {element}; //更新任务的子节点

  callback = callback === undefined ? null : callback; //任务执行完成后的回调
  if (callback !== null) {
    if (__DEV__) {
  ...
  const root = enqueueUpdate(current, update, lane); //更新任务压入队列返回rootFiber节点
  if (root !== null) {
    scheduleUpdateOnFiber(root, current, lane, eventTime); //调度更新任务
    entangleTransitions(root, current, lane); //TODO 开启transition
  }
